...
namespace sg
{
	namespace event_control
	{
		...
		{
			MouseEvent(const uint32_t screen_x, const uint32_t screen_y,
				const uint32_t window_x, const uint32_t window_y) :
				Event(Event::EventType::MouseEvent), screen_x(screen_x), screen_y(screen_y),
				window_x(window_x), window_y(window_y) {};
			const uint32_t screen_x, screen_y, window_x, window_y;
		};
		struct WindowEvent : Event
		{
			...
			WindowEvent(const WindowEventType type) :
				Event(Event::EventType::WindowEvent), window_event_type(type) {};
		};
	}
}